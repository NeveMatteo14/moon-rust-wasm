...
impl<'a> Env<'a> {
    pub fn new(buffer: Rc<RefCell<Vec<String>>>) -> Self {
        ...
        env.insert_global("print".to_string(), LuaValue::new(LuaVal::Print(buf_clone)));
    ...
    pub fn create_with_captured_env(&self, local_env: &LocalEnv<'a>) -> Env<'a> {
        let mut new_env = Env::new(self.print_buffer.clone());
